...
CPU_USE=`echo ${SYSTEM_IDLE} ${TOTAL_TIME} | awk '{printf "%.2f", 100-$1/$2*100}'`

MEM_USE=`free -m | sed -n '2p' | awk '{printf "%f\n",($3)/$2*100}'`
 
DISK_USE=`df | grep ^/dev | awk '{print $1":"$5}' |tr '\n' ';'`
...
echo "('"$time"','"$MYIP"',"$CPU_USE","$MEM_USE",'"$DISK_USE"')"
